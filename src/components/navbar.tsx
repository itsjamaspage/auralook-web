...
import { User, LogOut, LayoutDashboard, Menu, Heart } from 'lucide-react';
...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-4 lg:px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between glass-dark rounded-2xl px-4 lg:px-6 py-3 border border-white/10 shadow-2xl">
        <Link href="/" className="flex items-center gap-2 group">
          <span className="text-xl lg:text-2xl font-black tracking-tighter neon-text whitespace-nowrap italic group-hover:scale-105 transition-transform">
            Auralook.uz
          </span>
        </Link>

        <div className="hidden lg:flex items-center gap-8 text-sm font-medium">
          <Link href="/looks" className="text-white/80 hover:neon-text transition-colors uppercase tracking-widest font-black text-[10px]">{t(dictionary.browseLooks)}</Link>
        </div>

        <div className="flex items-center gap-2 lg:gap-4">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="rounded-full border border-white/20 hover:bg-white/5 h-9 lg:h-10 px-3 lg:px-4 font-black uppercase text-white text-[10px]">
                {lang}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="glass-dark border-white/10">
              {languages.map((l) => (
                <DropdownMenuItem 
                  ...
                  className={`font-bold text-xs ${lang === l.code ? "bg-white/10 neon-text" : "text-white"}`}
                >
                  {l.label}
                </DropdownMenuItem>
              ))}
            ...
            user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button 
                    variant="ghost" 
                    className="rounded-full font-black border border-white/20 hover:bg-white/5 transition-all bg-transparent px-3 lg:px-4 h-9 lg:h-10 text-white text-[10px] flex items-center gap-2"
                  >
                    <User className="w-4 h-4 text-primary" />
                    <span className="hidden lg:inline">{user.email?.split('@')[0]}</span>
                    {likedLooks && likedLooks.length > 0 && (
                      <div className="flex items-center gap-1 bg-primary/20 px-1.5 py-0.5 rounded-md border border-primary/30">
                        <Heart className="w-2.5 h-2.5 fill-primary text-primary" />
                        <span className="text-[9px] neon-text">{likedLooks.length}</span>
                      </div>
                    )}
                  </Button>
                </DropdownMenuTrigger>
              ...
              <div className="flex items-center gap-2">
                <Link href="/login">
                  <Button 
                    variant="ghost" 
                    className="rounded-full font-black border border-white/20 hover:bg-white/5 transition-colors bg-transparent px-3 lg:px-4 h-9 lg:h-10 text-white text-[10px]"
                  >

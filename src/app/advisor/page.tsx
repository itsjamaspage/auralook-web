...
export default function AdvisorPage() {
  ...
  return (
    <div className="container mx-auto px-6 py-8 space-y-8 max-w-2xl pb-32">
      ...
      <Card className="glass-surface border-foreground/10 p-8 space-y-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
        ...
        {!result ? (
          <div className="space-y-8 py-4 relative z-10">
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-3">
                <Label className="font-bold uppercase tracking-widest text-[10px] text-foreground">{t(dictionary.heightCmLabel)}</Label>
                <Input 
                  type="number" 
                  value={formData.height} 
                  onChange={(e) => setFormData({...formData, height: parseInt(e.target.value)})}
                  className="bg-foreground/5 border-foreground/10 h-12 rounded-xl focus:neon-border text-foreground transition-none"
                />
              </div>
              <div className="space-y-3">
                <Label className="font-bold uppercase tracking-widest text-[10px] text-foreground">{t(dictionary.weightKgLabel)}</Label>
                <Input 
                  type="number" 
            ...
            <div className="space-y-4">
              <Label className="font-bold uppercase tracking-widest text-[10px] text-foreground">{t(dictionary.knownSize)}</Label>
              <Input 
                placeholder={t(dictionary.sizeExample)}
                value={formData.knownSize}
                onChange={(e) => setFormData({...formData, knownSize: e.target.value})}
                className="bg-foreground/5 border-foreground/10 h-12 rounded-xl focus:neon-border text-foreground transition-none placeholder:text-foreground/20"
              />
            </div>

            <div className="space-y-4">
              <Label className="font-bold uppercase tracking-widest text-[10px] text-foreground">{t(dictionary.gender)}</Label>
              <RadioGroup value={formData.gender} onValueChange={(val: any) => setFormData({...formData, gender: val})} className="flex gap-8">
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="male" id="male" className="transition-none border-foreground/30" />
                  <Label htmlFor="male" className="text-xs font-bold text-foreground">{t(dictionary.male)}</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="female" id="female" className="transition-none border-foreground/30" />
                  <Label htmlFor="female" className="text-xs font-bold text-foreground">{t(dictionary.female)}</Label>
                </div>
              </RadioGroup>
            </div>

            <div className="space-y-4">
              <Label className="font-bold uppercase tracking-widest text-[10px] text-foreground">{t(dictionary.fitStyle)}</Label>
              <RadioGroup value={formData.fit} onValueChange={(val: any) => setFormData({...formData, fit: val})} className="flex flex-col gap-4">
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="tight" id="tight" className="transition-none border-foreground/30" />
                  <Label htmlFor="tight" className="text-xs font-bold text-foreground">{t(dictionary.tight)}</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="regular" id="reg" className="transition-none border-foreground/30" />
                  <Label htmlFor="reg" className="text-xs font-bold text-foreground">{t(dictionary.regular)}</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="loose" id="loose" className="transition-none border-foreground/30" />
                  <Label htmlFor="loose" className="text-xs font-bold text-foreground">{t(dictionary.loose)}</Label>
                </div>
              </RadioGroup>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 pt-4">
              ...
              <Button 
                ...
                disabled={loading || submitting}
                variant="outline"
                className="w-full h-14 rounded-2xl border-foreground/10 text-foreground font-bold text-sm uppercase tracking-widest hover:neon-border hover:neon-text transition-none"
              >
          ...
          <div className="py-8 text-center space-y-8 relative z-10">
            <div className="inline-flex items-center justify-center w-28 h-28 rounded-full bg-foreground/5 border-2 neon-border animate-pulse mb-2 transition-none">
            ...
            <div className="space-y-4">
              ...
              <p className="text-foreground text-sm leading-relaxed px-6 italic font-medium">
                "{result.explanation}"
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <Button 
                  ...
                  variant="ghost" 
                  className="w-full rounded-xl border border-foreground/10 text-foreground hover:text-foreground transition-all font-bold uppercase tracking-widest text-[10px]"
              >
